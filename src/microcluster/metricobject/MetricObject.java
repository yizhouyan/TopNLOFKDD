package microcluster.metricobject;
...
public class MetricObject implements Comparable {
	...
	public float MinDistToMicroClusters(HashMap<Integer, MicroCluster> mcList, int currentMCId,
			HashMap<Integer, Boolean> overLapWithCurrentMC, IMetricSpace metricSpace, Set<Integer> neighborsForMC)
			throws IOException {
		...
		// Cluster
		for (Integer neighbor : neighborsForMC) {
			if (neighbor == currentMCId) {
				for (MetricObject mo : mcList.get(neighbor).getPointList()) {
					...
					indexOfMCs.add(neighbor);
				}
				// continue;
			} else {
				boolean overlap = overLapWithCurrentMC.get(neighbor);
				if (overlap) {
					float tempMinDist = this.minDistToMicroClusterOverlap(
							((Record) mcList.get(neighbor).getClusterCoor()).getValue(),
					...
					float tempMinDist = this.minDistToMicroClusterNoOverlap(mcList.get(neighbor), metricSpace);
					minDistanceToMCs.add(tempMinDist);
					indexOfMCs.add(neighbor);
				}
			}
	...
	public float MinDistToMicroClustersNoLocal(HashMap<Integer, MicroCluster> mcList, int currentMCId,
			...
			HashMap<Long, Float> localNeighbors, Set<Integer> neighborsForMC) throws IOException {
		ArrayList<Float> minDistanceToMCs = new ArrayList<Float>();
		ArrayList<Integer> indexOfMCs = new ArrayList<Integer>();
	...
	public float MaxDistToMicroClustersNoLocal(HashMap<Integer, MicroCluster> mcList, int currentMCId,
			IMetricSpace metricSpace, HashMap<Long, Float> localNeighbors, Set<Integer> neighborsForMC)
			throws IOException {
		...
		for (Integer neighbor : neighborsForMC) {
			if (neighbor == currentMCId) {
				...
				float tempMaxDist = this.maxDistToMicroCluster(mcList.get(neighbor), metricSpace);
	...
	public float MaxDistToMicroClusters(HashMap<Integer, MicroCluster> mcList, int currentMCId,
			IMetricSpace metricSpace, Set<Integer> neighborsForMC) throws IOException {
		ArrayList<Float> maxDistanceToMCs = new ArrayList<Float>();
		ArrayList<Integer> indexOfMCs = new ArrayList<Integer>();
		// get min and max distance from the point to the corresponding Micro
		// Cluster
